...
-- function check_fixing_machines() -- older, broken implementation
--   log("HELLO - script")
--   log(serpent.block(storage.fixing_machines))
--   log(serpent.block(storage.nullius_broken_status))
...
function broken_finished_delayed(name)
  if storage.fixing_machines ~= nil and storage.fixing_machines[name] ~= nil then -- maybe superfluous
    return
  end
  ...
  for _, machine in pairs(machines) do
    ...
    if recipe ~= nil and recipe.name == name then
      if storage.fixing_machines == nil then storage.fixing_machines = {} end
      if storage.fixing_machines[name] ~= nil then -- todo: do something about duplicates ?
        table.insert(storage.fixing_machines[name], machine)
      else
        storage.fixing_machines[name] = {machine}
      end
    end
  end
end
...
function check_fixing_machines()
  if storage.fixing_machines == nil then return end
  for recipe_name, machines in pairs(storage.fixing_machines) do
    local recipe_finished = true
    for _, machine in pairs(machines) do
      local recipe = machine.get_recipe()
      if recipe ~= nil and recipe.name == recipe_name then
        recipe_finished = false
        break
      end
    end
    if recipe_finished then
      broken_finished(recipe_name)
      storage.fixing_machines[recipe_name] = nil
    end
  end
